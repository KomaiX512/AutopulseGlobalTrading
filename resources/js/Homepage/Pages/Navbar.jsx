...
const Navbar = ({ auth }) => {
    ...
    return (
        <div className="bg-gray-800 p-0 relative z-50 pb-1 mobile-optimized">
            <Toaster position="top-right" />
            <Cart auth={auth} open={open} onClose={() => setOpen(false)} />
            
            {/* Enhanced Mobile Navigation */}
            <div className="nav-up flex justify-between items-center text-white nav-up-content" style={{ padding: '0.5vw' }}>
                ...
                {/* Mobile Contact Info */}
                <div className="flex items-center gap-4 hide-1200 mobile-contact-info">
                    <Tooltip title={'Phone number'} className="text-sm flex items-center gap-1 contact-item">
                        <FaWhatsapp size={20} />
                        <a href={`https://wa.me/13072950382?`} style={{ fontWeight: '500' }}>
                            +1307 2950382
                        </a>
                    </Tooltip>
                    <Tooltip title={'Email Us'} className="text-sm flex items-center gap-1 contact-item">
                        <GrMail size={20} />
                ...
                <div className="hidden unhide-1200 flex items-center gap-4">
                    <div style={{ gap: '3vw' }} className="hidden flex items-center icons-container unhide-750 hide-1200 mx-1">
                        {location.pathname.split('/')[1] === 'parts' && (
                            <Badge count={state?.cart?.cartItems?.length} className='flex items-center gap-3 hide-750'>
                        ...
                        {location.pathname.split('/')[1] === 'parts' && (
                            <a href="/track" className="text-gray-800 flex icon-container hide-1200 items-center gap-1">
            ...
            <div id='nav-fix' className="container nav-down flex justify-between items-center gap-3 bg-white search-container" style={{ borderRadius: "30px", padding: "1.5vw" }}>
                ...
                <div className="flex gap-4 items-center justify-between sm-gap-3" style={{ width: "100%" }}>
                    ...
                    <div style={{ marginRight: "3vw" }} className="d-none unhide-750 sm-gap-3 flex gap-4 items-center hide-1200">
                        {location.pathname.split('/')[1] === 'parts' && (
                            <Badge count={state?.cart?.cartItems?.length} className='flex items-center gap-3'>
                    ...
                    <div className="flex items-center gap-4 icons-container hide-750">
                        ...
                        {!auth.user && (
                            <a href="/register" className="primary-btn text-gray icon-container flex items-center gap-1">
                                Register
                            </a>
                        )}
