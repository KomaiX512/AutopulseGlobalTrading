...
function RelatedAttachments({ header = true }) {
    ...
    useEffect(() => {
        if (state?.selectedAttachment?.slug) {
    ...
    return (
        <div className="latest-products container p-3 bg-white mt-3" style={{ padding: '30px 12px' }}>
            <div className="text-center mb-8 py-2 pb-4 flex align-center justify-between px-1">
                <h2 className="text-secondary text-lg font-semibold">Related Attachments</h2>
            ...
            <div className="container mx-auto p-0" style={{ position: 'relative' }}>
                ...
                <div className=" pr-0" style={{ paddingRight: '0px !important' }}>
                    <Slider ref={sliderRef} {...settings}>
                        {state?.relatedAttachments?.map((attachment, index) => (
                            <AttachmentComponent key={attachment.id} attachment={attachment} index={index} />
                        ))}
                    </Slider>
                </div>
            </div>
